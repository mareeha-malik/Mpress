...
class PostForm(forms.ModelForm):
    class Meta:
        model = Post
        fields = ['title', 'category', 'tags', 'content', 'featured_image', 'status']
        widgets = {
            ...
            'status': forms.Select(attrs={'class': 'px-3 py-2 rounded-xl bg-white/90 border border-white/30 text-gray-800'}),
        }
